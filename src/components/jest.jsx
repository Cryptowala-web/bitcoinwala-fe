...
const JelloText = () => {
  ...
  const updateCharDimensions = (dragScale) => {
    setChars(prevChars =>
      prevChars.map((char, index) => {
        ...
        const elasticWeight = Math.max(100, weightInit - (fracDispersion * weightDiff * 1.2));
        const elasticStretch = Math.max(50, stretchInit - (fracDispersion * stretchDiff * 1.2));
        return {
          ...char,
          y: elasticY,
  ...
  const setElementProps = (element, props) => {
    Object.keys(props).forEach(prop => {
      if (prop === 'x' || prop === 'y') {
        ...
        const otherMatch = currentTransform.match(otherProp === 'x' ? /translateX\(([^)]+)\)/ : /translateY\(([^)]+)\)/);
  ...
  const animateElement = (element, props, duration, delay = 0, ease = 'ease-out') => {
      ...
      setTimeout(() => {
        ...
        Object.keys(props).forEach(prop => {
          if (prop === 'x' || prop === 'y') {
            ...
            const match = transform.match(prop === 'x' ? /translateX\(([^)]+)\)/ : /translateY\(([^)]+)\)/);
        ...
        const animate = (currentTime) => {
          ...
          const easeProgress = ease === 'ease-out' ? 1 - Math.pow(1 - progress, 3) : progress;
